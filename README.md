...
- **秘传升级计算器**：野心 2→3 缺 6 级秘传时，按你手上的碎片给出最短路径
  （同类升级 / 拗转牺牲上游系 / 从零收集），机制核对自游戏配方
- **远征作战计划**：藏宝地在场时逐个障碍列出克制系，并按你手下的切面
  等级给出成功率（必成 / 约七成 / 约三成 / 硬性失败）
...
- **一局史复盘**：按角色（存档建档时间）记录整局历史，从新档到结局——
  ...
  重启续记；附资源趋势图、统计摘要和"行为统计"面板（本局每种行动做了
  多少次，取自存档的配方执行计数）
- **分层建议面板**：⚠ 紧急（红底加粗）/ ● 建议 / ○ 情报三档视觉分级，
  时节预报、牌库构成等背景信息灰字缩小垫底，危机永远醒目在顶
- **剧透等级**（尊重克苏鲁式的未知体验）：**守密人**只报生存警报，命运的
  抉择留给你自己；**顾问**（默认）给引导但不预告抉择后果——远征障碍侦察后
  才显示、时节牌库只报张数、遗赠等命运时刻只标记不剧透；**全知**开天眼，
  隐藏牌堆、抉择后果、书商存货尽收眼底。被收起的提示会显示条数，随时可调
